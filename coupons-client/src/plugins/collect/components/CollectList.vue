		...
		<block v-for="(item, index) in list" :key="item.collectionId">
			<view class="cart-item solid-bottom" @tap="navToDetailPage(item)">
				<view class="image-wrapper2">
					...
					<image :src="item.data.mainPic || item.data.marketingMainPic" :class="[item.loaded]"
						mode="aspectFill" lazy-load @load="onImageLoad('list', index)"
				...
				<view class="item-right">
					...
					<view>
						<view class="flex align-center justify-between">
							...
							<text class="juan" v-if="(item.data.couponPrice-0)>0"><text
									class="text">劵</text>{{item.data.couponPrice - 0}}元</text>
						</view>
						<view class="salse">
							<view class="text">30天销量:{{item.data.monthSales}}</view>
							<view class="text" v-if="item.data.couponReceiveNum>0">领券量:{{item.data.couponReceiveNum}}
							</view>
						</view>
						<view class="flex align-center justify-end">
							<view class="cu-btn sm margin-top-sm">
								<text class="icon text-lg text-black icon-delete_light"
									@tap.stop="deleteItem(index, item)"></text>
							</view>
						</view>
...
<style scoped>
	...
	.cart-item .image-wrapper2 image {
		width: 100%;
		height: 100%;
		border-radius: 8upx;
	}
