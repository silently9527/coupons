			...
			<block slot="custom">
				<view class="cu-bar search bg-white" style="width: 100%;">
					<view class="search-form padding-left-xs">
						...
						<input class="margin-left-xs" v-model="keyword" type="search" @input="inputChange"
							@confirm="doSearch(false)" :placeholder="defaultKeyword" />
					</view>
				</view>
		...
		<view class="search-keyword">
			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				...
				<view class="keyword-block">
					<view class="keyword-list-header solid-bottom">
						<view>热门搜索</view>
						<view @tap="hotToggle"><text class="icon text-grey"
								:class="!forbid?'icon-attention':'icon-attentionforbid'"></text></view>
					</view>
					...
					<view class="hide-hot-tis padding-top-xs" v-else>
		...
		methods: {
			...
			async loadHotKeyword() {
				...
				if (this.hotKeywordList.length === 0) {
					let response = await api.getHotSearch();
			...
			async doSearch(key) {
				...
				uni.navigateTo({
					url: `/plugins/search/pages/result?keyword=${key}`
				});
			},
			//保存关键字到历史记录
			saveKeyword(keyword) {
