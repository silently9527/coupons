	...
	<view class="container">
		...
		<view v-if="isRecommend" :style="[{marginTop: CustomBar+'px'}]">
			<refresh @onRefresh="onRecommendRefresh" :pullupLoadingType="loadingType">
				<collocation-card v-for="item in collocations" :key="item.collocationId" :item="item"
					@appreciate="appreciate" @detail="navCollocationDetail" />
			</refresh>
		</view>
	...
	export default {
		...
		methods: {
			...
			async loadRecommendData(type) {
				let result = await api.recommendCollocations(this.page++);
			...
			async loadCollocationList(type) {
				let result = await api.collocationsList(this.page2++, this.sex);
				let list = result.data.content || [];
				if (type === 'refresh') {
			...
			appreciate(item) {
				...
				if (item2.appreciate) {
					api.cancelCollocationAppreciate(item2.collocationId).then(() => {
						item2.appreciate = false;
						item2.appreciateCount -= 1;
					});
				} else {
					api.addCollocationAppreciate(item2.collocationId).then(() => {
