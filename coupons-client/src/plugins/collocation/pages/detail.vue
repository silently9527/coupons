	...
	<view class="container">
		...
		<view class="introduce">
			<view class="flex justify-between align-center">
				<view>
					<text class="icon icon-attentionfill text-gray text-sm margin-right-xs"> {{detail.viewCount}}
						浏览</text>
					<text class="padding-left icon icon-appreciatefill text-sm"
						:class="detail.appreciate?'text-red':'text-gray'"> {{detail.appreciateCount}} 赞</text>
				</view>
				<view class="text-sm">
			...
			<view class="tags margin-top-sm">
				<view v-for="tag in detail.tags" :key="tag.tagId" class='cu-tag line-gray radius padding-sm'>
					{{tag.name}}
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
			return {
				title: this.detail.description.substring(0, this.detail.description.length > 20 ? 20 : this.detail
					.description.length),
		...
		methods: {
			async loadCollocationDetail(collocationId) {
				let result = await api.collocationSimpleDetail(collocationId);
				this.detail = result.data
				api.collocationProduct(collocationId).then((response) => {
					this.products = response.data
				});
			},

			async recommendCollocations2() {
				api.recommendCollocations2(this.page++).then((response) => {
