	...
	<view class="container">
		...
		<view class="introduce-section">
			...
			<view class="price-box">
				...
				<text class="m-price" v-if="detail.actualPrice !== detail.originalPrice">原价
		...
		<view v-if="detail.couponPrice > 0" style="background-color: white;padding-bottom:20upx">
			<view class="coupon">
				<view class="div-left">
					...
					<view style="font-size: 11px">{{detail.couponStartTime | formatDate}} ~
						{{detail.couponEndTime | formatDate}}
					</view>
				</view>
		...
		<view class="page-bottom">
			...
			<view class="p-b-btn" @tap="toFavorite()">
				<text class="icon text-xxl" :class="{'icon-favor': !favorite, 'icon-favorfill text-red': favorite}">
				</text>
				<text>收藏</text>
			</view>
			<!-- <view class="p-b-btn">
		...
		<!-- 规格-模态层弹窗 -->
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="stopPrevent" @click="toggleTpwd">
			<!-- 遮罩层 -->
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				<view class="a-t">
					...
					<view class="right margin-top-sm">
						<view>
							<text v-if="detail.couponPrice > 0" class="margin-right-xs"
								style="font-size: 22upx;color: red;">券后</text>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			api.getPrivilegeLink(goodsId).then(response => {
			...
			let result = await api.getGoodsDetail(null, goodsId)
			if (!result.success) {
				uni.showToast({
					title: '该商品已下架',
					icon: 'none',
		...
		methods: {
			...
			toFavorite() {
				...
				if (this.favorite) {
					api.unFavoriteGoods(this.detail.goodsId)
				} else {
...
<style scoped>
	...
	.store-info .store-evaluate {
		margin-top: 10upx;
	}

	.store-info .store-evaluate text {
